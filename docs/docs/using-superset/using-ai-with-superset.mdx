...
## Gantt charts through MCP

Use `chart_type: "gantt"` with temporal `start_time` and `end_time` column
references and a `category` task column. Optional `series` colors the intervals;
`subcategories: true` separates tasks within each series. Ask
`get_chart_type_schema` for the complete Gantt configuration and examples.

Gantt Vega-Lite previews skip rows with null or invalid bounds, reversed intervals,
or missing category values. An empty query result produces an empty preview;
a nonempty result with no renderable intervals returns an error. Missing result
fields and malformed result structures still return errors. Preview specifications
contain at most 1,000 renderable rows and only fields used by the visualization.

When updating a saved or cached Gantt chart, omitting `filters` preserves existing
non-generated adhoc filters. Supplying `filters`, including `[]`, replaces them.
The mapper-owned temporal binding is regenerated from the supplied configuration;
an inactive native `No filter` placeholder is replaced by that binding. Unsupported
merged filter state is rejected rather than silently discarded. Native sort entries
must remain valid `[column, ascending_boolean]` pairs.
