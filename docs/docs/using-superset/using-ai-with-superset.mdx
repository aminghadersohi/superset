...
an optional secondary color metric. Use `chart_type: "sunburst"` in typed requests;
the native chart uses `viz_type: "sunburst_v2"`.
...
Chart queries also accept Superset's `cache_timeout = -1` setting to disable caching.
